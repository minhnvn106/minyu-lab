 ...
 *
 *   1. `endpoint`  — the pretty URL the user pastes (/betterlinks/mcp), with the
 *                    connection token. Depends on rewrite rules, so it fails on
 *                    plain permalinks or an unflushed rule table.
 *   2. `fallback`  — the always-on /wp-json/betterlinks/v1/mcp route. Works even
...
final class Mcp_Self_Test {
	...
	public static function run(): array {
		...
		if ( ! $result['mcp_enabled'] ) {
			...
			$result['message'] = __( 'MCP access is turned off, so the endpoint refuses every request. Enable MCP access above and try again.', 'betterlinks' );
			return $result;
		}

		if ( ! $result['connected'] ) {
			$result['stage']   = 'not_connected';
			$result['message'] = __( 'No connection token exists yet. Click Connect to mint one, then run the test again.', 'betterlinks' );
			return $result;
		}
		...
		$result['checks'][] = self::check( 'endpoint', __( 'Connection URL', 'betterlinks' ), $pretty['stage'], $pretty['detail'] );
		$result['checks'][] = self::check( 'fallback', __( 'REST fallback URL', 'betterlinks' ), $rest['stage'], $rest['detail'] );
		...
		$result['checks'][]            = self::check( 'discovery', __( 'OAuth discovery', 'betterlinks' ), $discovery['stage'], $discovery['detail'] );

		$challenge          = self::probe_challenge( $endpoint, $fallback );
		$result['checks'][] = self::check( 'challenge', __( 'OAuth challenge', 'betterlinks' ), $challenge['stage'], $challenge['detail'] );
		...
		if ( null !== $user_agent ) {
			$result['checks'][] = self::check( 'user_agent', __( 'Client access', 'betterlinks' ), $user_agent['stage'], $user_agent['detail'] );
		}
		...
		if ( null !== $lockouts && $lockouts > 0 ) {
			$result['checks'][] = self::check(
				'lockouts',
				__( 'Client lockouts', 'betterlinks' ),
				'locked_clients',
				sprintf(
		...
		if ( 'ok' !== $pretty['stage'] && 'ok' === $rest['stage'] ) {
			...
			$result['message'] = sprintf(
				...
				__( 'The connection URL %1$s did not answer, but the REST fallback %2$s works. Re-save Settings → Permalinks to rebuild the rewrite rules; until then, give your AI client the fallback URL.', 'betterlinks' ),
				$endpoint,
				$fallback
			);
			return $result;
	...
	private static function probe_jsonrpc( string $url, string $token ): array {
		...
		if ( is_wp_error( $response ) ) {
			...
			$out['detail'] = $is_tls
				...
				? sprintf( __( '%1$s could not be reached over HTTPS: %2$s. On local/dev sites this is usually a self-signed certificate the AI client must be told to trust.', 'betterlinks' ), $url, $err )
				/* translators: 1: endpoint URL, 2: underlying transport error. */
				: sprintf( __( '%1$s could not be reached: %2$s.', 'betterlinks' ), $url, $err );
			return $out;
		}
		...
		if ( in_array( $status, [ 301, 302, 307, 308 ], true ) ) {
			...
			$out['detail'] = $location
				...
				? sprintf( __( '%1$s redirected to %2$s instead of answering. A redirect between HTTP and HTTPS usually means the site address and WordPress address schemes disagree.', 'betterlinks' ), $url, $location )
				/* translators: %s: endpoint URL. */
				: sprintf( __( '%s redirected instead of answering, which usually means the site address and WordPress address schemes disagree.', 'betterlinks' ), $url );
			return $out;
		}
		...
		if ( 404 === $status ) {
			...
			$out['detail'] = sprintf(
				/* translators: %s: endpoint URL. */
				__( '%s returned 404 — WordPress does not know this URL. Re-save Settings → Permalinks to rebuild the rewrite rules.', 'betterlinks' ),
				$url
			);
			return $out;
		}
		...
		if ( 401 === $status || 403 === $status ) {
			...
			$out['detail'] = sprintf(
				/* translators: %s: endpoint URL. */
				__( '%s rejected the connection token (authentication failed). Rotate the token and reconnect your AI client.', 'betterlinks' ),
				$url
			);
			return $out;
		}
		...
		if ( 429 === $status ) {
			...
			$out['detail'] = sprintf(
				/* translators: %s: endpoint URL. */
				__( '%s is rate-limiting this server after repeated failed tokens. Wait for the lockout to lapse, then rotate the token and reconnect.', 'betterlinks' ),
				$url
			);
			return $out;
		}
		...
		// detail so support can tell "no BetterLinks abilities registered"
		...
		if ( 200 !== $status || null === $tools || [] === $tools ) {
			...
			$out['detail'] = sprintf(
				...
				__( '%1$s answered but returned no tool catalog. Confirm the MCP runtime is built and abilities are registered. Diagnostics — %2$s', 'betterlinks' ),
				$url,
		...
		$out['detail'] = sprintf(
			...
			__( '%1$s authenticated and returned %2$d tools.', 'betterlinks' ),
			$url,
			count( $tools )
		);
		return $out;
	...
	private static function probe_discovery(): array {
			...
			if ( is_wp_error( $response ) ) {
				return [
					...
					'detail'    => sprintf(
						...
						__( 'The OAuth discovery document %1$s could not be fetched: %2$s. Clients that connect by URL alone cannot authenticate without it.', 'betterlinks' ),
						$url,
			...
			if ( 200 !== $status || ! is_array( $body ) || ! isset( $body[ $spec['key'] ] ) ) {
				return [
					...
					'detail'    => sprintf(
						...
						__( 'The OAuth discovery document %1$s returned %2$d instead of valid metadata. Re-save Settings → Permalinks; if it persists, another plugin may be claiming the /.well-known/ URLs.', 'betterlinks' ),
						$url,
						$status
					),
				];
			...
			if ( $advertised !== $spec['expected'] ) {
				return [
					...
					'detail'    => sprintf(
						...
						__( 'The discovery document %4$s advertises %1$s "%2$s" but this site\'s MCP endpoint is "%3$s". RFC 9728 requires an exact match, so clients reject the metadata and report that the server does not implement OAuth. If the two differ only by scheme, a reverse proxy is terminating TLS without passing X-Forwarded-Proto; otherwise another plugin is serving this URL.', 'betterlinks' ),
						$spec['key'],
						$advertised,
						$spec['expected'],
						$url
		...
		return [
			'stage'     => 'ok',
			'documents' => $documents,
			'detail'    => __( 'Both OAuth discovery documents are served and advertise this site\'s MCP endpoint exactly.', 'betterlinks' ),
		];
	}
	...
	private static function probe_scheme(): ?array {
		...
		return [
			...
			'detail' => sprintf(
				...
				__( 'The discovery documents advertise %1$s, but %2$s answers as well — WordPress is storing an http:// site address behind a proxy that terminates TLS. AI clients connect over https and reject the http identifier as a mismatch. Fix the Site Address in Settings → General, or have the proxy send X-Forwarded-Proto.', 'betterlinks' ),
				$endpoint,
				$secure
			),
		];
	...
	private static function probe_challenge( string $endpoint, string $fallback ): array {
		...
		foreach ( [ $endpoint, $fallback ] as $url ) {
			...
			if ( 401 !== $status ) {
				return [
					...
					'detail' => sprintf(
						...
						__( 'An unauthenticated call to %1$s answered %2$d instead of 401. Clients that connect by URL alone need the 401 challenge to start the OAuth flow.', 'betterlinks' ),
						$url,
						$status
					),
				];
			...
			if ( '' === $challenge ) {
				return [
					...
					'detail' => sprintf(
						/* translators: %s: endpoint URL. */
						__( '%s answered 401 but sent no WWW-Authenticate header — a security plugin or proxy is likely stripping it. Clients that connect by URL alone will report that this server does not implement OAuth.', 'betterlinks' ),
						$url
					),
				];
			}
			...
			if ( ! preg_match( '/resource_metadata="([^"]+)"/i', $challenge, $m ) ) {
				return [
					...
					'detail' => sprintf(
						...
						__( '%1$s sent a WWW-Authenticate header with no resource_metadata URL (%2$s). Clients have nowhere to look up this site\'s OAuth metadata.', 'betterlinks' ),
						$url,
						$challenge
					),
				];
		...
		if ( ! $answered ) {
			return [
				'stage'  => 'challenge',
				'detail' => __( 'The OAuth challenge could not be checked because the endpoint did not answer. Fix the connection error above and re-run the test.', 'betterlinks' ),
			];
		}

		return [
			'stage'  => 'ok',
			'detail' => __( 'Unauthenticated calls answer with the OAuth challenge, so URL-only clients can authenticate.', 'betterlinks' ),
		];
	}
	...
	private static function probe_user_agent( string $endpoint ): ?array {
		...
		foreach ( self::CLIENT_USER_AGENTS as $agent ) {
			...
			return [
				...
				'detail' => sprintf(
					...
					__( 'The endpoint answered %3$d for WordPress but %2$d for an AI client\'s User-Agent (%1$s). A security plugin, firewall or "block bad bots" rule is refusing non-browser clients — exempt the MCP and /.well-known/ paths, or no AI client will ever reach this site.', 'betterlinks' ),
					$agent,
					$status,
					$baseline
				),
		...
		return [
			'stage'  => 'ok',
			'detail' => __( 'The endpoint answers AI-client User-Agents the same way it answers WordPress, so no bot filter is blocking them. This cannot see an IP-level block of the AI vendor.', 'betterlinks' ),
		];
	}
