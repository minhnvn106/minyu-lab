 ...
 *
 * State lives in the `betterlinks_mcp_oauth` option (clients, codes, tokens,
...
/**
 * Minimal OAuth 2.1 authorization server for the BetterLinks MCP endpoint.
 */
final class Mcp_OAuth {
	 ...
	 */
	public const OPTION = 'betterlinks_mcp_oauth';
	 ...
	 * (/.well-known/oauth-authorization-server/betterlinks/mcp), which stay
	 * specific to BetterLinks even when another plugin runs its own MCP OAuth
	 * server at the same site root.
	 *
	 * @return string
	 */
	public static function issuer(): string {
		return untrailingslashit( home_url( '/betterlinks/mcp' ) );
	}
	...
	public static function authorize_url(): string {
		return home_url( '/betterlinks/authorize' );
	}
	...
	public static function token_url(): string {
		return rest_url( 'betterlinks/v1/mcp/oauth/token' );
	}
	...
	public static function register_url(): string {
		return rest_url( 'betterlinks/v1/mcp/oauth/register' );
	}
	...
	public static function register_client( array $body ) {
		...
		if ( empty( $redirect_uris ) ) {
			return new \WP_Error(
				...
				__( 'At least one valid redirect_uri is required.', 'betterlinks' ),
				[ 'status' => 400 ]
			);
		}
	...
	public static function validate_authorize_request( array $params ) {
		...
		if ( null === $client ) {
			return new \WP_Error( 'invalid_client', __( 'Unknown client_id.', 'betterlinks' ), [ 'status' => 400 ] );
		}
		if ( ! in_array( $redirect_uri, $client['redirect_uris'], true ) ) {
			// redirect_uri mismatch must NOT redirect (open-redirect guard).
			return new \WP_Error( 'invalid_redirect_uri', __( 'redirect_uri does not match a registered value.', 'betterlinks' ), [ 'status' => 400 ] );
		}
		if ( 'code' !== $response_type ) {
			return new \WP_Error(
				'unsupported_response_type',
				__( 'Only response_type=code is supported.', 'betterlinks' ),
				[
					'status'       => 400,
					'redirectable' => true,
				]
		...
		if ( 'S256' !== $method || '' === $challenge ) {
			return new \WP_Error(
				'invalid_request',
				__( 'PKCE with code_challenge_method=S256 is required.', 'betterlinks' ),
				[
					'status'       => 400,
					'redirectable' => true,
				]
	...
	public static function connected_apps(): array {
		...
		foreach ( $active as $cid => $info ) {
			...
			$apps[] = [
				'client_id'    => $cid,
				'name'         => isset( $client['name'] ) ? (string) $client['name'] : __( 'MCP Client', 'betterlinks' ),
				'scope'        => $info['scope'],
