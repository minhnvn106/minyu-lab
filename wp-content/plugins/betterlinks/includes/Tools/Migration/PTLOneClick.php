...
class PTLOneClick extends BaseCSV {
	...
	public function insert_link( $link_id ) {
		global $wpdb;
		$item = $wpdb->get_row(
			$wpdb->prepare( "SELECT * FROM {$wpdb->prefix}prli_links WHERE id=%s LIMIT 1", $link_id ),
			ARRAY_A
		);
		...
		$link = apply_filters(
			'betterlinks/tools/migration/ptl_one_click_import_link_arg',
			array(
		...
		if ( $link_id ) {
			...
			if ( isset( $item['link_cpt_id'] ) && ! empty( $item['link_cpt_id'] ) ) {
				...
				$terms_ids = \BetterLinks\Helper::insert_category_terms( array( $term ) );
