 ...
 *
 * Threshold + window are overridable via the BETTERLINKS_MCP_MAX_FAILS /
 * BETTERLINKS_MCP_LOCKOUT_SECONDS constants and the `betterlinks_mcp_rate_limit`
...
final class Mcp_Rate_Limiter {
	 ...
	 */
	private const PREFIX = 'betterlinks_mcp_rl_';
	...
	private static function limits(): array {
		$max    = defined( 'BETTERLINKS_MCP_MAX_FAILS' ) ? (int) \BETTERLINKS_MCP_MAX_FAILS : self::DEFAULT_MAX_FAILS;
		$window = defined( 'BETTERLINKS_MCP_LOCKOUT_SECONDS' ) ? (int) \BETTERLINKS_MCP_LOCKOUT_SECONDS : self::DEFAULT_LOCKOUT;
		 ...
		 */
		$limits = (array) apply_filters( 'betterlinks_mcp_rate_limit', [ $max, $window ] );
	...
	private static function client_ip(): string {
		 ...
		 */
		$ip = (string) apply_filters( 'betterlinks_mcp_client_ip', $ip );
