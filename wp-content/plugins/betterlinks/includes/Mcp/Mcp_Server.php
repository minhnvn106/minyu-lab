 ...
 * (https://thissite.com/betterlinks/mcp), so there is NO hosted broker in the
...
/**
 * JSON-RPC 2.0 handler for the BetterLinks MCP endpoint.
 */
final class Mcp_Server {
	...
	public static function handle( \WP_REST_Request $request ): \WP_REST_Response {
		// Diagnostic tap: define BETTERLINKS_MCP_DEBUG in wp-config.php to log
		...
		if ( ! Mcp_Manager::is_enabled() ) {
			return self::error_response( null, self::UNAUTHORIZED, 'MCP is disabled on this site. Enable it under BetterLinks → MCP.', 403 );
		}
			...
			case 'initialize':
				return self::result(
					...
					[
						...
						'serverInfo'      => [
							'name'    => 'betterlinks',
							'version' => defined( 'BETTERLINKS_VERSION' ) ? BETTERLINKS_VERSION : '1.0.0',
						],
					]
				);
